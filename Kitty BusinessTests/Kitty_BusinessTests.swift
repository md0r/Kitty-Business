...
class test_the_login_view_model: XCTestCase {
    ...
    @MainActor func test_should_detect_empty_credentials_when_trying_to_login() async {
        ...
        //in an application with a real backend, you want to use a different mock/fake backend service for your UNIT tests with a local db or something like that, so that you don't have to wait for a response from the actual server
    ...
    @MainActor func test_should_login_user_with_correct_credentials() async {
        ...
        //this variable needs to change to TRUE if the logins are correct
        loginViewModel.navigateToNextView = false
        
        //this method tests the logins and should change the navigateToNextView to TRUE if the logins are correct
